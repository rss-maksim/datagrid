  ...
  return (
    <>
      <div className={classnames('grid-cell header-grid-cell', classname)} style={style} onClick={onSort}>
        ...
        {isOrderByThis && <ArrowRightAlt className={classnames('sort-arrow-icon', { desc })} fontSize="small" />}
        {filterable && <SearchIcon className="search-icon" fontSize="small" onClick={onHandleToggle} />}
      </div>
      {filterable &&
        filterIsActive &&
